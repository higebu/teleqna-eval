...
type Task struct {
	ID           string          `json:"id"`
	Type         string          `json:"type"` // "asn1" or "formula"
	Question     string          `json:"question"`
	Gold         json.RawMessage `json:"gold"` // []string for asn1, string for formula
...
// GoldFields returns the expected ASN.1 field names in definition order.
func (t Task) GoldFields() []string {
	var f []string
...
func Grade(t Task, a Answer) Score {
	...
	switch t.Type {
	case "asn1":
		got, want := a.Fields(), t.GoldFields()
